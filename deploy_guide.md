...
# 將本地預設分支更名為 main（避免 Windows Git 預設的 master 分支導致推播失敗）
git branch -M main
...
2. **郵件通知設定 (Gmail API 發信)**：
   * 由於 Hugging Face 雲端封鎖了外發 SMTP Port (587)，本系統已移除 SMTP 密碼發信功能，強制使用更安全的 **Gmail API OAuth** 發信。
   * **啟用方式**：請依照第二階段的說明，在 Hugging Face 的 Secrets 中新增 `GMAIL_CREDENTIALS`，並且在 `GMAIL_USER` 設定您的 Gmail 帳號。
   * **網頁後台設定**：只需勾選 **"啟用測驗完成 Email 通知"**，並填寫 **"收件者信箱"** (多位家長/老師請用逗號隔開) 即可。不需要（也無法）在後台網頁中填寫寄件者或密碼，防止使用者亂填造成錯誤。
3. **儲存設定**：
   * 點選 **"💾 儲存所有設定"**，即刻完成部署！
